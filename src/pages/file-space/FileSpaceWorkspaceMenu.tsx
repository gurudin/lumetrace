...
export function FileSpaceWorkspaceStatus({ directory }: FileSpaceWorkspaceStatusProps) {
  ...
  if (external?.active) return <div className={`file-space-workspace-status${external.icon ? " has-icon" : ""}`}>
    <span aria-hidden="true" className={external.icon ? "file-space-workspace-status-icon" : undefined}>{external.icon}</span>
    <span><strong>{external.name}</strong><small>{external.typeLabel}</small></span>
  </div>;
