...
Lumie 基于当前文件空间执行 RAG（检索增强生成）：

1. Lume Trace 先在本地索引中检索有限数量的相关正文片段。
2. 只把这些片段、来源信息和最近对话上下文交给已配置的回答服务。
3. 回答及其引用的文件和版本保存在当前文件空间。
...
- 使用 AI 时，只有检索命中的片段会交给当前选择的本地服务、云端接口或 Agent CLI；不会在单次请求中发送整个文件空间。
