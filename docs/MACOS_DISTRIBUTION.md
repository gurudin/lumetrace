...
The next Community version is 1.0.3. Keep the existing release tags unchanged. The user-facing Release notes live in [releases/v1.0.3.md](releases/v1.0.3.md); download and installation text must be finalized against the actual installer before publication. Developer verification belongs here or in CI, not in the user-facing Release notes.

On 2026-09-14, the system keychain exposed no valid code-signing identity. Xcode's notarytool and stapler are available. The 1.0.3 build cannot be described as Developer ID signed, notarized, or Apple-verified unless a valid identity and notarization credentials become available before publication.
...
src-tauri/target/aarch64-apple-darwin/release/bundle/dmg/LumeTrace_1.0.3_aarch64.dmg
```

Confirm actual output paths. Do not rename an older-version binary to make it look like 1.0.3. Do not install development builds into `/Applications` or launch automated acceptance against the user's live workspace.
...
hdiutil verify 'path/to/LumeTrace_1.0.3_aarch64.dmg'
shasum -a 256 'path/to/LumeTrace_1.0.3_aarch64.dmg'
```
...
- Retain existing release tags unchanged. Agree on the exact v1.0.3 commit and tag before creating a new tag. Do not switch or merge main automatically.
- Prepare a draft Release with [v1.0.3 notes](releases/v1.0.3.md), the verified DMG, and its SHA-256 checksum. Verify the artifact name matches Download.
