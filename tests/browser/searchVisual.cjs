...
(async () => {
  ...
  try {
    for (const theme of ['light','dark']) for (const language of ['en','zh-CN']) for (const width of [920,1280]) {
      ...
      failPdf=false;
      await page.locator('.search-visual-preview [role=alert] button').focus();
      await page.keyboard.press('Enter');await current.first().waitFor();
      assert.equal(await page.locator('.file-space-global-search-panel').count(),1,'Enter activates Retry instead of opening the selected file');
