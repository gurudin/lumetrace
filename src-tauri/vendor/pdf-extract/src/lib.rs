#![allow(dead_code, hidden_glob_reexports, mismatched_lifetime_syntaxes, unused_variables)]
    ...
    fn process_stream(&mut self, doc: &'a Document, content: Vec<u8>, resources: &'a Dictionary, media_box: &MediaBox, output: &mut dyn OutputDev, page_num: u32) -> Result<(), OutputError> {
        ...
        for operation in &content.operations {
            ...
            match operation.operator.as_ref() {
                ...
                "gs" => {
                    if let Some(ext_gstate) = maybe_get::<&Dictionary>(doc, resources, b"ExtGState") {
                        if let Ok(name) = operation.operands[0].as_name() {
                            if let Some(state) = maybe_get::<&Dictionary>(doc, ext_gstate, name) {
